...
if not any("evo" in arg.lower() for arg in sys.argv):
    sys.argv.append("evo")
...
def main() -> None:
    ...
    assert (ACTION_DIM, PROPRIO_DIM, NUM_ACTIONS_CHUNK) == (29, 14, 50)
    ...
    assert config["input_features"]["observation.state"]["shape"] == [14]
    assert config["output_features"]["action"]["shape"] == [50, 29]

    stats_path = os.environ.get("COSMOS_STATS_PATH")
    if stats_path:
        stats = json.loads(Path(stats_path).read_text())
        assert np.asarray(stats["actions_min"]).shape == (29,)
        assert np.asarray(stats["proprio_min"]).shape == (14,)
